...
public class PageRequestDTO {
	...
	// 글읽기, 글수정, 글삭제 등에 페이지 번호와 검색 파라미터를 url에 계속 데리고 다녀야 합니다.
	// 이를 위해서 문자열 생성 메소드 정의합니다.
	private String link;	// url에 들어갈 파라미터 문자열

	//Getter는 처리 코드가 필요하여 직접 작성합니다.
	public String getLink() {
		if (link == null) {
			StringBuilder builder = new StringBuilder();
			builder.append("page=" + this.page);
			if (this.type != null && this.type.length() > 0 && keyword != null) {
			...
			if (this.from != null && this.to != null) {	// 아직 UI에는 미구현한 상태
		...
		return this.link;
		// 최종 link는 예시) page3&type=tc&keyword=hi&from=2023-03-01&to=2023-03-31
	}
